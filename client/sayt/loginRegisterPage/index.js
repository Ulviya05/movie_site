...
$(document).ready(function() {
    // Assuming user_id is a variable that holds the user's ID
  
    // Get a reference to the discussion link
    const discussionLink = $('a[href="../loginRegisterPage/index.html"]');
  
    // Add a click event listener to the discussion link
    discussionLink.on('click', function(event) {
      // Prevent the default link behavior
      event.preventDefault();
  
      // Check if user_id exists or is empty
      if (!user_id) {
        // Redirect to login.html
        window.location.href = '../loginRegisterPage/login.html';
      } else {
...
if(!user_id) {
    // window.location.href = '../loginRegisterPage/login.html';
    $(`#logged`).html(`
    <a href="../loginRegisterPage/login.html"><button class="click cLogin">Login</button></a>
    <a href="../loginRegisterPage/register.html"><button class="click cRegister">Register</button></a>
    `);
}
else {
    // Add a request interceptor
    $(`#logged`).html(`
    <div class="avatar_img"><img src=${ localStorage.getItem("image")} style="margin-top:-0.1px;width: 50px;height: 50px;margin-right: 20px; border-radius:50%; overflow:hidden;></div>
    <div class="icon"><ion-icon name="log-out-outline" style="color:grey; font-size:30px; overflow:hidden; cursor:pointer;" onclick="logout()"></ion-icon></div>
    </div>
    <script type="module" src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.esm.js%22%3E</script>
        <script nomodule src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.js%22%3E</script>`);
